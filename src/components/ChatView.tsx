...
import { sendPushNotification, ensureNotificationPermission } from '../utils/notifications';
...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  const isInitialChatsLoadedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const unsubscribeChats = onValue(chatsRef, (snapshot) => {
      ...
      Object.entries(allVal).forEach(([roomKey, roomData]: [string, any]) => {
        ...
        const msgList = Object.entries(roomData).map(([k, v]: [string, any]) => {
          ...
          if (k && typeof k === 'string') {
            ...
            if (isInitialChatsLoadedRef.current && isNew) {
              ...
              if (!isMe) {
                ...
                if (notifChatsEnabled) {
                  ...
                  sendPushNotification(`💬 ${senderLabel}`, { body: preview, icon: '/logo.svg' });
                }
              }
            }
          }
      ...
      isInitialChatsLoadedRef.current = true;
    });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(postsQuery, (snapshot) => {
          ...
          if (!loadedPostIdsRef.current.has(docSnap.id)) {
            ...
            if (prevCommentCount !== undefined && currentCommentCount > prevCommentCount) {
              ...
              if (lastComment && lastComment.authorName.toLowerCase() !== myName.toLowerCase()) {
                ...
                sendPushNotification(`💬 Reply from ${lastComment.authorName}`, { body: lastComment.text, icon: '/logo.svg' });
              }
            }
            ...
            if (prevUpvoteCount !== undefined && currentUpvoteCount > prevUpvoteCount) {
              ...
              sendPushNotification("💖 Post Liked!", { body: `Someone upvoted: "${postItem.title || postItem.content.substring(0, 30)}..."`, icon: '/logo.svg' });
            }
          }
        }
      ...
      if (newPostDetected) {
        ...
        sendPushNotification(`${categoryLabel}: ${newPostDetected.author.name}`, {
          body: newPostDetected.title || newPostDetected.content.substring(0, 45),
          icon: '/logo.svg'
        });
      }
    }, (err) => {
