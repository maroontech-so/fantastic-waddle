...
import { Library, MessageSquare, User as UserIcon, Settings, LogOut, Terminal, ChevronLeft, ChevronRight, GraduationCap, Download } from 'lucide-react';
      ...
      <nav className={`flex-1 ${isCollapsed ? 'px-2' : 'px-4.5'} py-6 space-y-1.5 overflow-y-auto relative z-10 no-scrollbar`}>
        ...
        {canInstallPWA && (
          <button
            ...
            title={isCollapsed ? "Install App / PWA" : undefined}
            className={`w-full flex items-center ${isCollapsed ? 'justify-center px-0' : 'gap-3 px-3.5'} py-2.5 rounded-xl transition-all duration-200 apple-active cursor-pointer text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 font-medium`}
          >
            <Download className="w-4.5 h-4.5 text-amber-500 shrink-0 animate-bounce" />
            {!isCollapsed && <span className="text-xs animate-fade-in whitespace-nowrap font-bold">Install PWA App</span>}
          </button>
        )}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 space-y-2">
          {canInstallPWA && (
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3 rounded-xl transition-all flex justify-center items-center gap-2 cursor-pointer text-xs shadow-md"
            >
              <Download className="w-4 h-4 animate-bounce" /> Install App / PWA
            </button>
          )}
          <button
            onClick={() => {
