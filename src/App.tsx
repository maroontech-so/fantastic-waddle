...
import { Bell, Search, Code2, ArrowRight, Loader2, Menu, AlertTriangle, Copy, Check, Info, Moon, Sun, Download } from 'lucide-react';
...
export function App() {
  ...
  useEffect(() => {
    ...
    (window as any).triggerPwaInstallModal = () => setShowPwaModal(true);

    return () => {
      window.removeEventListener('resize', checkStandalone);
      window.removeEventListener('beforeinstallprompt', handleBeforeInstallPrompt);
      delete (window as any).triggerPwaInstallModal;
    };
  }, []);
    ...
    if (deferredPrompt) {
      try {
        ...
        if (outcome === 'accepted') {
          ...
          triggerToast('🎉 Installing </AdvocoDe> PWA to your device...');
        } else {
          triggerToast('Install prompt cancelled.');
        }
      } catch (err) {
  ...
  return (
    <div className={`antialiased text-slate-900 flex h-screen w-full bg-slate-50 overflow-hidden font-sans ${textSize === 'large' ? 'text-scale-large' : textSize === 'larger' ? 'text-scale-larger' : 'text-scale-normal'}`}>
      ...
      {user && !splashActive && (
        <>
          ...
          <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-slate-50">
            ...
            {!isStandalone && !localStorage.getItem('advocode_pwa_banner_dismissed') && (
              <div className="bg-gradient-to-r from-blue-900/95 via-indigo-900/95 to-slate-900/95 text-white px-4 py-2 flex items-center justify-between gap-3 shrink-0 shadow-md relative z-20">
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="flex h-2 w-2 rounded-full bg-amber-400 animate-pulse shrink-0"></span>
                  <p className="text-xs font-medium truncate">
                    <span className="font-bold">📱 Download App:</span> Install as standalone PWA for background alerts & offline access!
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={handleInstallPWA}
                    className="bg-white text-blue-900 hover:bg-blue-50 font-bold px-3 py-1 rounded-lg text-xs shadow transition-colors cursor-pointer flex items-center gap-1.5"
                  >
                    <Download className="w-3 h-3 text-blue-600" /> Install
                  </button>
                  <button
                    onClick={() => {
                      localStorage.setItem('advocode_pwa_banner_dismissed', 'true');
                      triggerToast('Banner dismissed. You can always install from Sidebar or Profile!');
                    }}
      ...
      <PwaInstallModal
        ...
        onInstallDirect={handleInstallPWA}
      />
    </div>
  );
}
